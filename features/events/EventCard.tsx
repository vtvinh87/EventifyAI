 ...
 */
const EventCardComponent: React.FC<EventCardProps> = ({ event, disableLink }) => {
  const cardContent = (
    <Card className={`h-full flex flex-col ${!disableLink ? 'group-hover:!border-brand-500/50 hover:-translate-y-1 hover:scale-[1.02]' : ''}`}>
      <div className="overflow-hidden">
        <img
          src={event.bannerUrl}
          alt={event.name}
          className={`w-full h-40 object-cover ${!disableLink ? 'group-hover:scale-110' : ''} transition-transform duration-300 ease-in-out`}
        />
      </div>
      <div className="p-4 flex flex-col flex-grow">
        <p className="text-sm font-medium text-brand-400">{event.category.toUpperCase()}</p>
        <h3 className={`text-xl font-bold text-white mt-1 mb-2 flex-grow ${!disableLink ? 'group-hover:text-brand-300' : ''} transition-colors`}>{event.name}</h3>
